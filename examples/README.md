# Beads Examples

This directory contains examples of how to integrate beads with AI agents and workflows.

## Examples

- **[python-agent/](python-agent/)** - Simple Python agent that discovers ready work and completes tasks
- **[bash-agent/](bash-agent/)** - Bash script showing the full agent workflow
- **[markdown-to-jsonl/](markdown-to-jsonl/)** - Convert markdown planning docs to beads issues
- **[github-import/](github-import/)** - Import issues from GitHub repositories
- **[git-hooks/](git-hooks/)** - Pre-configured git hooks for automatic export/import
- **[claude-desktop-mcp/](claude-desktop-mcp/)** - MCP server for Claude Desktop integration
- **[claude-code-skill/](claude-code-skill/)** - Claude Code skill for effective beads usage patterns

## Quick Start

```sh
# Try the Python agent example
cd python-agent
uv run agent.py

# Try the bash agent example
cd bash-agent
./agent.sh

# Install git hooks
cd git-hooks
./install.sh

# REMOVED (beads-4c74): branch-merge demo - hash IDs eliminate collision resolution
```

## Creating Your Own Agent

The basic agent workflow:

1. **Find ready work**: `beads ready --json --limit 1`
2. **Claim the task**: `beads update <id> --status in_progress --json`
3. **Do the work**: Execute the task
4. **Discover new issues**: `beads create "Found bug" --json`
5. **Link discoveries**: `beads dep add <new-id> <parent-id> --type discovered-from`
6. **Complete the task**: `beads close <id> --reason "Done" --json`

All commands support `--json` for easy parsing.
