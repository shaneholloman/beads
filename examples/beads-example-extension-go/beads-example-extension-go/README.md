# BEADS Extension Example (Go)

This example demonstrates how to extend beads with custom tables for application-specific orchestration, following the patterns described in [extending.md](../../docs/extending.md).

## What This Example Shows

1. **Schema Extension**: Adding custom tables (`example_executions`, `example_checkpoints`) to beads's SQLite database
2. **Foreign Key Integration**: Linking extension tables to beads's `issues` table with proper cascading
3. **Dual-Layer Access**: Using beads's Go API for issue management while directly querying extension tables
4. **Complex Queries**: Joining beads's issues with extension tables for powerful insights
...
Extension tables link to beads's issues with cascading deletes:
...
- **beads layer**: Issue tracking, dependencies, ready work
...
- Go 1.21 or later
- beads initialized in a directory (run `beads init --prefix demo`)
...
cd examples/beads-example-extension-go
go install .
```

The binary will be installed as `beads-example-extension-go` in your `$GOPATH/bin` (or `$GOBIN` if set).
...
beads-example-extension-go -db .beads/demo.db
```
...
2. Dual-layer access (beads API + direct SQL)
...
### Using beads's Go API

```go
// Auto-discover database path
dbPath := beads.FindDatabasePath()
if dbPath == "" {
    log.Fatal("No beads database found")
}
...
See [extending.md](../../docs/extending.md) for more patterns and the VC implementation example.
...
5. **Integrate with Agents**: Use beads's ready work queue for agent orchestration

## See Also

- [extending.md](../../docs/extending.md) - Complete extension guide
- [../../README.md](../../README.md) - beads documentation
- Run `beads quickstart` for an interactive tutorial
