...
func TestCheckVersionMismatch_NoVersion(t *testing.T) {
	...
	version, err := sqliteStore.GetMetadata(ctx, "beads_version")
	if err != nil {
...
func TestCheckVersionMismatch_SameVersion(t *testing.T) {
	...
	// Set same version
	if err := sqliteStore.SetMetadata(ctx, "beads_version", Version); err != nil {
...
func TestCheckVersionMismatch_OlderBinary(t *testing.T) {
	...
	if err := sqliteStore.SetMetadata(ctx, "beads_version", "99.99.99"); err != nil {
		t.Fatalf("Failed to set version: %v", err)
	}
...
func TestCheckVersionMismatch_NewerBinary(t *testing.T) {
	...
	if err := sqliteStore.SetMetadata(ctx, "beads_version", "0.1.0"); err != nil {
		t.Fatalf("Failed to set version: %v", err)
	}
