...
import (
	...
	"github.com/shaneholloman/beads/internal/daemon"
	"github.com/shaneholloman/beads/internal/rpc"
	"github.com/shaneholloman/beads/internal/storage"
...
Use --health to check daemon health and metrics.`,
	Run: func(cmd *cobra.Command, args []string) {
			...
			if isRunning, pid := isDaemonRunning(pidFile); isRunning {
				...
				if client, err := rpc.TryConnectWithTimeout(socketPath, 1*time.Second); err == nil && client != nil {
					...
					if healthErr == nil && health.Compatible {
						...
						fmt.Fprintf(os.Stderr, "Use 'beads daemon --stop%s' to stop it first\n", boolToFlag(global, " --global"))
						os.Exit(1)
					}
		...
		fmt.Printf("Starting beads daemon (%s, interval: %v, auto-commit: %v, auto-push: %v)\n",
...
func init() {
	...
	daemonCmd.Flags().Bool("global", false, "Run as global daemon (socket at ~/.beads/beads.sock)")
...
func ensureBeadsDir() (string, error) {
	...
	if dbPath != "" {
		...
		if foundDB := beads.FindDatabasePath(); foundDB != "" {
			...
			return "", fmt.Errorf("no database path configured (run 'beads init' or set BEADS_DB)")
		}
	}
...
func getSocketPathForPID(pidFile string, global bool) string {
	if global {
		...
		return filepath.Join(home, ".beads", "beads.sock")
	}
	// Local daemon: socket is in same directory as PID file
	return filepath.Join(filepath.Dir(pidFile), "beads.sock")
}
...
func showDaemonHealth(global bool) {
	...
	if global {
		...
		socketPath = filepath.Join(home, ".beads", "beads.sock")
	} else {
		beadsDir, err := ensureBeadsDir()
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error: %v\n", err)
...
func migrateToGlobalDaemon() {
	...
	cmd := exec.Command(binPath, "daemon", "--global") // #nosec G204 - beads daemon command from trusted binary
...
This usually means:
  ...
  2. Git remote URL changed (run 'beads migrate --update-repo-id')
  3. Database corruption
  4. beads was upgraded and URL canonicalization changed

Solutions:
  - If remote URL changed: beads migrate --update-repo-id
  - If beads was upgraded: beads migrate --update-repo-id
  - If wrong database: rm -rf .beads && beads init
  - If correct database: BEADS_IGNORE_REPO_MISMATCH=1 beads daemon
...
func runGlobalDaemon(log daemonLogger) {
	...
	socketPath := filepath.Join(globalDir, "beads.sock")
...
func createExportFunc(ctx context.Context, store storage.Storage, autoCommit, autoPush bool, log daemonLogger) func() {
	return func() {
		...
		if autoCommit {
			...
			if hasChanges {
				message := fmt.Sprintf("beads daemon export: %s", time.Now().Format("2006-01-02 15:04:05"))
...
func createSyncFunc(ctx context.Context, store storage.Storage, autoCommit, autoPush bool, log daemonLogger) func() {
	return func() {
		...
		if autoCommit {
			...
			if hasChanges {
				message := fmt.Sprintf("beads daemon sync: %s", time.Now().Format("2006-01-02 15:04:05"))
...
func runDaemonLoop(interval time.Duration, autoCommit, autoPush bool, logPath, pidFile string, global bool) {
	...
	if !global {
		...
		if daemonDBPath == "" {
			if foundDB := beads.FindDatabasePath(); foundDB != "" {
				...
				log.log("Hint: run 'beads init' to create a database or set BEADS_DB environment variable")
				os.Exit(1)
			}
		}
	}
	...
	if err == nil && len(matches) > 1 {
		...
		if len(validDBs) > 1 {
			...
			log.log("Run 'beads init' to migrate legacy databases")
			os.Exit(1)
		}
	}
	...
	if dbBaseName != beads.CanonicalDatabaseName {
		...
		log.log("")
		log.log("Run 'beads init' to migrate to canonical name")
		os.Exit(1)
	}
	...
	dbVersion, err := store.GetMetadata(versionCtx, "beads_version")
	if err != nil && err.Error() != "metadata key not found: beads_version" {
	...
	if dbVersion != "" && dbVersion != Version {
		...
		log.log("")
		log.log("The database was created with a different version of beads.")
		log.log("This may cause compatibility issues.")
		log.log("")
		log.log("Options:")
		log.log("  1. Run 'beads migrate' to update the database to the current version")
		log.log("  2. Upgrade/downgrade beads to match database version: %s", dbVersion)
		...
		if err := store.SetMetadata(versionCtx, "beads_version", Version); err != nil {
