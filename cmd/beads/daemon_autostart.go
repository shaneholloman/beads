...
func determineSocketMode(socketPath string) (string, bool) {
	...
	globalSocket := filepath.Join(home, ".beads", "beads.sock")
...
// Always returns local socket path (.beads/beads.sock relative to database)
func getSocketPath() string {
	// Always use local socket (same directory as database: .beads/beads.sock)
	localSocket := filepath.Join(filepath.Dir(dbPath), "beads.sock")
	...
	if home, err := os.UserHomeDir(); err == nil {
		...
		if _, err := os.Stat(globalSocket); err == nil {
			...
			fmt.Fprintf(os.Stderr, "To migrate: Stop the global daemon and restart with 'beads daemon' in each project.\n")
		}
	}
...
func emitVerboseWarning() {
	...
	case FallbackConnectFailed:
		fmt.Fprintf(os.Stderr, "Warning: Daemon unreachable at %s. Running in direct mode. Hint: beads daemon --status\n", daemonStatus.SocketPath)
	case FallbackHealthFailed:
		fmt.Fprintf(os.Stderr, "Warning: Daemon unhealthy. Falling back to direct mode. Hint: beads daemon --health\n")
	case FallbackAutoStartDisabled:
		fmt.Fprintf(os.Stderr, "Warning: Auto-start disabled (BEADS_AUTO_START_DAEMON=false). Running in direct mode. Hint: beads daemon\n")
	case FallbackAutoStartFailed:
		fmt.Fprintf(os.Stderr, "Warning: Failed to auto-start daemon. Running in direct mode. Hint: beads daemon --status\n")
