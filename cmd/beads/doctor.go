...
Examples:
  beads doctor              # Check current directory
  beads doctor /path/to/repo # Check specific repository
  beads doctor --json       # Machine-readable output`,
...
func checkInstallation(path string) doctorCheck {
	...
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		...
		return doctorCheck{
			...
			Fix:     fmt.Sprintf("Run 'beads init --prefix %s' to initialize beads", prefix),
		}
	}
...
func checkDatabaseVersion(path string) doctorCheck {
	...
	if _, err := os.Stat(dbPath); os.IsNotExist(err) {
		...
		return doctorCheck{
			...
			Fix:     "Run 'beads init' to create database",
		}
	}
	...
	if dbVersion == "unknown" {
		return doctorCheck{
			...
			Fix:     "Database may be corrupted. Try 'beads migrate'",
		}
	}
	...
	if dbVersion == "pre-0.17.5" {
		return doctorCheck{
			...
			Detail:  "Storage: SQLite",
			Fix:     "Run 'beads migrate' to upgrade database schema",
		}
	}
	...
	if dbVersion != Version {
		return doctorCheck{
			...
			Detail:  "Storage: SQLite",
			Fix:     "Run 'beads migrate' to sync database with CLI version",
		}
	}
...
func checkIDFormat(path string) doctorCheck {
	...
	return doctorCheck{
		Name:    "Issue IDs",
		Status:  statusWarning,
		Message: "sequential (e.g., beads-1, beads-2, ...)",
		Fix:     "Run 'beads migrate --to-hash-ids' to upgrade (prevents ID collisions in multi-worker scenarios)",
	}
}
...
func checkCLIVersion() doctorCheck {
	...
	if compareVersions(latestVersion, Version) > 0 {
		upgradeCmds := `  • Homebrew: brew upgrade beads
  • Script: curl -fsSL https://raw.githubusercontent.com/shaneholloman/beads/main/scripts/install.sh | bash`
...
func getDatabaseVersionFromPath(dbPath string) string {
	...
	var version string
	err = db.QueryRow("SELECT value FROM metadata WHERE key = 'beads_version'").Scan(&version)
	if err == nil {
		return version
	}
...
func fetchLatestGitHubRelease() (string, error) {
	url := "https://api.github.com/repos/shaneholloman/beads/releases/latest"
