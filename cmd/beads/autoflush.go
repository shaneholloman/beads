...
// Fixes beads-84: Hash-based comparison is git-proof (mtime comparison fails after git pull)
// Fixes beads-228: Now uses collision detection to prevent silently overwriting local changes
func autoImportIfNewer() {
	...
	// Check for Git merge conflict markers (beads-270)
	...
	for _, line := range lines {
		...
		if bytes.HasPrefix(trimmed, []byte("<<<<<<< ")) ||
			...
			fmt.Fprintf(os.Stderr, "     beads export -o %s\n\n", jsonlPath)
	...
	// Use shared import logic (beads-157)
	opts := ImportOptions{
	...
	// Store import timestamp (beads-159: for staleness detection)
...
func checkVersionMismatch() {
	...
	if dbVersion == "" {
		_ = store.SetMetadata(ctx, "beads_version", Version)
		return
	}
	...
	if dbVersion != Version {
		...
		fmt.Fprintf(os.Stderr, "\n%s\n", yellow("WARNING: WARNING: Version mismatch detected!"))
		fmt.Fprintf(os.Stderr, "%s\n", yellow(fmt.Sprintf("WARNING: Your beads binary (v%s) differs from the database version (v%s)", Version, dbVersion)))
		...
		if cmp < 0 {
			...
			fmt.Fprintf(os.Stderr, "%s\n", yellow("WARNING: Your binary appears to be OUTDATED."))
			fmt.Fprintf(os.Stderr, "%s\n\n", yellow("WARNING: Some features may not work correctly. Rebuild: go build -o beads ./cmd/beads"))
		} else if cmp > 0 {
			// Binary is newer than database
			fmt.Fprintf(os.Stderr, "%s\n", yellow("WARNING: Your binary appears NEWER than the database."))
			fmt.Fprintf(os.Stderr, "%s\n\n", yellow("WARNING: The database will be upgraded automatically."))
...
// If mismatch detected, clears export_hashes and logs warning (beads-160).
func validateJSONLIntegrity(ctx context.Context, jsonlPath string) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			fmt.Fprintf(os.Stderr, "WARNING: WARNING: JSONL file missing but export_hashes exist. Clearing export_hashes.\n")
			if err := store.ClearAllExportHashes(ctx); err != nil {
	...
	if currentHash != storedHash {
		fmt.Fprintf(os.Stderr, "WARNING: WARNING: JSONL file hash mismatch detected (beads-160)\n")
...
func writeJSONLAtomic(jsonlPath string, issues []*types.Issue) ([]string, error) {
	...
	// Create temp file with PID suffix to avoid collisions (beads-306)
	...
	// Write all issues as JSONL (timestamp-only deduplication DISABLED - beads-160)
...
// warning suggesting manual "beads export" to recover. Failure counter resets on success.
//
// Thread-safety:
...
func flushToJSONL() {
	...
	recordFailure := func(err error) {
		...
		if failCount >= 3 {
			...
			fmt.Fprintf(os.Stderr, "\n%s\n", red("WARNING: CRITICAL: Auto-flush has failed "+fmt.Sprint(failCount)+" times consecutively!"))
			fmt.Fprintf(os.Stderr, "%s\n", red("WARNING: Your JSONL file may be out of sync with the database."))
			fmt.Fprintf(os.Stderr, "%s\n\n", red("WARNING: Run 'beads export -o .beads/issues.jsonl' manually to fix."))
		}
	}
	...
	// Validate JSONL integrity before export (beads-160)
	...
	if fullExport {
		...
		// Incremental export: get only dirty issue IDs (beads-39 optimization)
	...
	// Clear only the dirty issues that were actually exported (fixes beads-52 race condition, beads-159)
	...
	// Store hash of exported JSONL (fixes beads-84: enables hash-based auto-import)
	jsonlData, err := os.ReadFile(jsonlPath)
	if err == nil {
		hasher := sha256.New()
		hasher.Write(jsonlData)
		...
		// Store JSONL file hash for integrity validation (beads-160)
