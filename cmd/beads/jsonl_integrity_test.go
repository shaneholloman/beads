...
// TestJSONLIntegrityValidation tests the JSONL integrity validation (beads-160)
func TestJSONLIntegrityValidation(t *testing.T) {
	...
	if err := testStore.SetConfig(ctx, "issue_prefix", "beads"); err != nil {
	...
	t.Run("MismatchedHash", func(t *testing.T) {
		...
		if err := os.WriteFile(jsonlPath, []byte(`{"id":"beads-1","title":"Modified"}`+"\n"), 0644); err != nil {
			t.Fatalf("failed to modify JSONL: %v", err)
		}

		// Add an export hash to verify it gets cleared
		if err := testStore.SetExportHash(ctx, "beads-1", "dummy-hash"); err != nil {
		...
		hash, err := testStore.GetExportHash(ctx, "beads-1")
		if err != nil {
...
// TestImportClearsExportHashes tests that imports clear export_hashes (beads-160)
