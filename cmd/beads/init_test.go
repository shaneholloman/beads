...
func TestInitCommand(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			prefix:         "",
			quiet:          false,
			wantOutputText: "beads initialized successfully",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			version, err := store.GetMetadata(ctx, "beads_version")
			if err != nil {
				t.Errorf("Failed to get beads_version metadata: %v", err)
			}
			if version == "" {
				t.Error("beads_version metadata was not set")
			}
		})
	}
}
