...
import (
	...
	"github.com/shaneholloman/beads/internal/storage/memory"
...
func TestExtractIssuePrefix(t *testing.T) {
	tests := []struct {
		...
		{"standard ID", "beads-123", "beads"},
		{"custom prefix", "myproject-456", "myproject"},
		{"hash ID", "beads-abc123def", "beads"},
		{"no hyphen", "nohyphen", ""},
		{"empty", "", ""},
	}
...
func TestLoadIssuesFromJSONL(t *testing.T) {
	...
	content := `{"id":"beads-1","title":"Test Issue 1","description":"Test"}
{"id":"beads-2","title":"Test Issue 2","description":"Another test"}

{"id":"beads-3","title":"Test Issue 3","description":"Third test"}
`
	...
	if issues[0].ID != "beads-1" || issues[0].Title != "Test Issue 1" {
		t.Errorf("First issue mismatch: %+v", issues[0])
	}
	if issues[1].ID != "beads-2" {
		t.Errorf("Second issue ID mismatch: %s", issues[1].ID)
	}
	if issues[2].ID != "beads-3" {
...
func TestLoadIssuesFromJSONL_InvalidJSON(t *testing.T) {
	...
	content := `{"id":"beads-1","title":"Valid"}
invalid json here
{"id":"beads-2","title":"Another valid"}
`
	if err := os.WriteFile(jsonlPath, []byte(content), 0o600); err != nil {
		t.Fatalf("Failed to write test file: %v", err)
	}
...
func TestWriteIssuesToJSONL(t *testing.T) {
	...
	issues := []*types.Issue{
		{ID: "beads-1", Title: "Test Issue 1", Description: "Desc 1"},
		{ID: "beads-2", Title: "Test Issue 2", Description: "Desc 2"},
	}
	if err := memStore.LoadFromIssues(issues); err != nil {
		t.Fatalf("Failed to load issues: %v", err)
	}
