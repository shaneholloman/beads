...
func TestMigrateHashIDs(t *testing.T) {
	...
	issue1 := &types.Issue{
		ID:          "beads-1",
		Title:       "First issue",
		Description: "This is issue beads-1",
	...
	issue2 := &types.Issue{
		ID:          "beads-2",
		Title:       "Second issue",
		Description: "This is issue beads-2 which references beads-1",
		Status:      types.StatusOpen,
		Priority:    1,
		IssueType:   types.TypeTask,
	}
	...
	if _, ok := mapping["beads-1"]; !ok {
		t.Error("Mapping missing beads-1")
	}
	if _, ok := mapping["beads-2"]; !ok {
		t.Error("Mapping missing beads-2")
	}
	...
	// Verify migration
	newID1 := mapping["beads-1"]
	newID2 := mapping["beads-2"]
...
func TestMigrateHashIDsWithParentChild(t *testing.T) {
	...
	child := &types.Issue{
		ID:          "beads-2",
		Title:       "Child issue",
		Description: "This is a child of beads-1",
		Status:      types.StatusOpen,
		Priority:    1,
		IssueType:   types.TypeTask,
	}
...
func TestIsHashID(t *testing.T) {
	tests := []struct {
		...
		{"beads-abc123", true},
		{"beads-123abc", true},
		{"beads-a3f8e9a2.1", true},
		{"beads-a3f8e9a2.1.2", true},
	}
