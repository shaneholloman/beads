...
func profileImportOperation(t *testing.T, numIssues int) {
	...
	tmpDir, err := os.MkdirTemp("", "beads-import-profile-*")
	if err != nil {
	...
	// REMOVED (beads-c7af): Counter sync - no longer needed with hash IDs
	...
	// Check performance targets from beads-199
	...
	if totalDuration > targetTime {
		t.Logf("\nWARNING: WARNING: Import took %v, target was %v", totalDuration, targetTime)
		t.Logf("This exceeds the performance target from beads-199")
	} else {
...
func TestImportWithExistingData(t *testing.T) {
	...
	numIssues := 208 // The exact number from beads-199

	// Create temp directory
	tmpDir, err := os.MkdirTemp("", "beads-import-existing-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)
	...
	if duration > 5*time.Second {
		t.Logf("\nWARNING: WARNING: Idempotent import took %v, expected < 5s", duration)
		t.Logf("This matches the symptoms described in beads-199")
	}
}
