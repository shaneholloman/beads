...
Example:
  beads detect-pollution                 # Show potential test issues
  beads detect-pollution --clean         # Delete test issues (with confirmation)
  beads detect-pollution --clean --yes   # Delete without confirmation
  beads detect-pollution --json          # Output in JSON format`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		if daemonClient != nil {
			...
			fmt.Fprintf(os.Stderr, "Use: beads --no-daemon detect-pollution\n")
			os.Exit(1)
		}
		...
		if !clean {
			fmt.Printf("Run 'beads detect-pollution --clean' to delete these issues (with confirmation).\n")
			return
		}
		...
		fmt.Printf("\nCleanup complete. To restore, run: beads import %s\n", backupPath)
	},
}
