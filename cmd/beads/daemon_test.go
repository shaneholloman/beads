...
func makeSocketTempDir(t testing.TB) string {
	...
	tmpDir, err := os.MkdirTemp(base, "beads-test-*")
	if err != nil {
	...
	tests := []struct {
	...
	}{
		{
			...
			set: func(t *testing.T) (string, string, string) {
				userDir := t.TempDir()
				dbDir := t.TempDir()
				userPath := filepath.Join(userDir, "beads.log")
...
// Once beads-85 (event-driven daemon) is fully implemented and enabled by default,
...
func TestMutationToExportLatency(t *testing.T) {
	...
	t.Skip("Skipping until event-driven daemon (beads-85) is fully implemented")
