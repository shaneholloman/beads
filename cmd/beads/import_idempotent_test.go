...
// does not update their timestamps (beads-84)
func TestIdempotentImportNoTimestampChurn(t *testing.T) {
	// Create temp directory
	tmpDir, err := os.MkdirTemp("", "beads-test-idempotent-*")
	if err != nil {
	...
	issue1, err := testStore.GetIssue(ctx, "beads-1")
	if err != nil {
	...
	// Get issue again
	issue2, err := testStore.GetIssue(ctx, "beads-1")
	if err != nil {
...
// does not update any of their timestamps (beads-84)
func TestImportMultipleUnchangedIssues(t *testing.T) {
	// Create temp directory
	tmpDir, err := os.MkdirTemp("", "beads-test-changed-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)
	...
	unchanged, _ := testStore.GetIssue(ctx, "beads-1")
	changed, _ := testStore.GetIssue(ctx, "beads-2")
	...
	issue1After, _ := testStore.GetIssue(ctx, "beads-1")
	issue2After, _ := testStore.GetIssue(ctx, "beads-2")

	// beads-1 should have same timestamp
	if !issue1After.UpdatedAt.Equal(unchangedInitialTS) {
		t.Errorf("beads-1 timestamp changed even though issue unchanged!\n"+
			"Before: %v\nAfter:  %v",
			unchangedInitialTS, issue1After.UpdatedAt)
	}

	// beads-2 should also have same timestamp
	if !issue2After.UpdatedAt.Equal(changedInitialTS) {
		t.Errorf("beads-2 timestamp changed even though issue unchanged!\n"+
			"Before: %v\nAfter:  %v",
