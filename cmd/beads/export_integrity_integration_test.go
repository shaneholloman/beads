...
// TestExportIntegrityAfterJSONLTruncation simulates the beads-160 bug scenario.
...
func TestExportIntegrityAfterJSONLTruncation(t *testing.T) {
	...
	// Step 2: Simulate git operation that truncates JSONL (the beads-160 scenario)
	...
	if len(exportedIDs2) != numIssues {
		...
		t.Errorf("This indicates the bug beads-160 would have occurred!")
	...
	if lineCount != numIssues {
		...
		t.Errorf("Data loss detected - this is the beads-160 bug!")
	}
}
