...
func TestOutputJSON(t *testing.T) {
	...
	// Verify content
	if result["id"] != "beads-1" {
		t.Errorf("Expected id 'beads-1', got '%v'", result["id"])
	}
...
func TestOutputJSONArray(t *testing.T) {
	...
	testData := []map[string]string{
		{"id": "beads-1", "title": "First"},
		{"id": "beads-2", "title": "Second"},
	}
...
// These functions no longer exist after refactoring to shared importIssuesCore (beads-157)

// Note: createIssuesFromMarkdown is tested via cmd/beads/markdown_test.go which has
