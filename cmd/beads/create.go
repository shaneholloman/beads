...
var createCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if parentID != "" {
			...
			if daemonClient != nil {
				// TODO: Add RPC support for GetNextChildID (beads-171)
		...
		// Supports: prefix-number (beads-42), prefix-hash (beads-a3f8e9), or hierarchical (beads-a3f8e9.1)
		if explicitID != "" {
			// Must contain hyphen
			if !strings.Contains(explicitID, "-") {
				fmt.Fprintf(os.Stderr, "Error: invalid ID format '%s' (expected format: prefix-hash or prefix-hash.number, e.g., 'beads-a3f8e9' or 'beads-a3f8e9.1')\n", explicitID)
				os.Exit(1)
			}
...
func init() {
	...
	createCmd.Flags().String("id", "", "Explicit issue ID (e.g., 'beads-42' for partitioning)")
	createCmd.Flags().String("parent", "", "Parent issue ID for hierarchical child (e.g., 'beads-a3f8e9')")
	createCmd.Flags().String("external-ref", "", "External reference (e.g., 'gh-9', 'jira-ABC')")
	createCmd.Flags().StringSlice("deps", []string{}, "Dependencies in format 'type:id' or 'id' (e.g., 'discovered-from:beads-20,blocks:beads-15' or 'beads-20')")
