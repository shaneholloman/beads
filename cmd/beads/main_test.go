...
func TestAutoFlushDebounce(t *testing.T) {
	// FIXME(beads-159): Test needs fixing - config.Set doesn't override flush-debounce properly
	t.Skip("Test needs fixing - config setup issue with flush-debounce")

	// Create temp directory for test database
	tmpDir, err := os.MkdirTemp("", "beads-test-autoflush-*")
	if err != nil {
...
func TestAutoFlushOnExit(t *testing.T) {
	// Create temp directory for test database
	tmpDir, err := os.MkdirTemp("", "beads-test-exit-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer func() {
...
func TestAutoFlushStoreInactive(t *testing.T) {
	// Create temp directory for test database
	tmpDir, err := os.MkdirTemp("", "beads-test-inactive-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer func() {
...
func TestAutoFlushJSONLContent(t *testing.T) {
	// Create temp directory for test database
	tmpDir, err := os.MkdirTemp("", "beads-test-content-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer func() {
...
func TestAutoFlushErrorHandling(t *testing.T) {
	...
	// Create temp directory for test database
	tmpDir, err := os.MkdirTemp("", "beads-test-error-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer func() {
...
func TestAutoImportIfNewer(t *testing.T) {
	// Create temp directory for test database
	tmpDir, err := os.MkdirTemp("", "beads-test-autoimport-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer func() {
...
func TestAutoImportDisabled(t *testing.T) {
	// Create temp directory for test database
	tmpDir, err := os.MkdirTemp("", "beads-test-noimport-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer func() {
...
func TestAutoImportWithUpdate(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "beads-test-update-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
...
func TestAutoImportNoUpdate(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "beads-test-noupdate-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)
...
// TestAutoImportMergeConflict tests that auto-import detects Git merge conflicts (beads-270)
func TestAutoImportMergeConflict(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "beads-test-conflict-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)
...
func TestAutoImportClosedAtInvariant(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "beads-test-invariant-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)
...
// beads-206: Test updating open issue to closed preserves closed_at
func TestImportOpenToClosedTransition(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "beads-test-open-to-closed-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)
	...
	testStore := newTestStoreWithPrefix(t, dbPath, "beads")

	ctx := context.Background()

	// Step 1: Create an open issue in the database
	openIssue := &types.Issue{
		ID:          "beads-transition-1",
	...
	err = testStore.UpdateIssue(ctx, "beads-transition-1", updates, "test")
	if err != nil {
		t.Fatalf("Update failed: %v", err)
	}

	// Step 3: Verify the issue is now closed with correct closed_at
	updated, err := testStore.GetIssue(ctx, "beads-transition-1")
	if err != nil {
...
// beads-206: Test updating closed issue to open clears closed_at
func TestImportClosedToOpenTransition(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "beads-test-closed-to-open-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)
	...
	err = testStore.UpdateIssue(ctx, "beads-transition-2", updates, "test")
	if err != nil {
		t.Fatalf("Update failed: %v", err)
	}

	// Step 3: Verify the issue is now open with null closed_at
	updated, err := testStore.GetIssue(ctx, "beads-transition-2")
	if err != nil {
		t.Fatalf("Failed to get updated issue: %v", err)
	}
