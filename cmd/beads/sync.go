...
Use --import-only to just import from JSONL (useful after git pull).`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		if jsonlPath == "" {
			fmt.Fprintf(os.Stderr, "Error: not in a beads workspace (no .beads directory found)\n")
			os.Exit(1)
		}
		...
		if inMerge, err := gitHasUnmergedPaths(); err != nil {
			...
			fmt.Fprintf(os.Stderr, "Hint: resolve conflicts, run 'beads import' if needed, then 'beads sync' again\n")
			os.Exit(1)
		}
		...
		if !noPull && !gitHasUpstream() {
			...
			fmt.Fprintf(os.Stderr, "Hint: git push -u origin <branch-name> (then rerun beads sync)\n")
			os.Exit(1)
		}
		...
		if !noPull {
			if dryRun {
				...
				if err := gitPull(ctx); err != nil {
					...
					fmt.Fprintf(os.Stderr, "Hint: resolve conflicts manually and run 'beads import' then 'beads sync' again\n")
					os.Exit(1)
				}
				...
				if hasPostImportChanges {
					...
					if err := gitCommit(ctx, jsonlPath, "beads sync: apply rename detection from import"); err != nil {
		...
		if !noPush && hasChanges {
			if dryRun {
				...
				if err := gitPush(ctx); err != nil {
					...
					fmt.Fprintf(os.Stderr, "Hint: pull may have brought new changes, run 'beads sync' again\n")
					os.Exit(1)
				}
			}
		}
...
func gitCommit(ctx context.Context, filePath string, message string) error {
	...
	if message == "" {
		message = fmt.Sprintf("beads sync: %s", time.Now().Format("2006-01-02 15:04:05"))
	}
...
func importFromJSONL(ctx context.Context, jsonlPath string, renameOnImport bool) error {
	// Get current executable path to avoid "./beads" path issues
	...
	// Run import command
	cmd := exec.CommandContext(ctx, exe, args...) // #nosec G204 - beads import command from trusted binary
	output, err := cmd.CombinedOutput()
	if err != nil {
