...
import (
	...
	"github.com/shaneholloman/beads/internal/config"
	"github.com/shaneholloman/beads/internal/configfile"
...
var initCmd = &cobra.Command{
	...
	Long: `Initialize beads in the current directory by creating a .beads/ directory
...
`
		...
		// Store the beads version in metadata (for version mismatch detection)
		if err := store.SetMetadata(ctx, "beads_version", Version); err != nil {
		...
		if issueCount > 0 {
			if !quiet {
				green := color.New(color.FgGreen).SprintFunc()
				fmt.Fprintf(os.Stderr, "\n%s Database initialized. Found %d issues in git, importing...\n", green("✔"), issueCount)
			}

			if err := importFromGit(ctx, initDBPath, store, jsonlPath); err != nil {
				if !quiet {
					fmt.Fprintf(os.Stderr, "Warning: auto-import failed: %v\n", err)
					fmt.Fprintf(os.Stderr, "Try manually: git show HEAD:%s | beads import -i /dev/stdin\n", jsonlPath)
				}
				// Non-fatal - continue with empty database
			} else if !quiet {
				green := color.New(color.FgGreen).SprintFunc()
				fmt.Fprintf(os.Stderr, "%s Successfully imported %d issues from git.\n\n", green("✔"), issueCount)
			}
		}
		...
		fmt.Printf("\n%s beads initialized successfully!\n\n", green("✔"))
...
// hooksInstalled checks if beads git hooks are installed
func hooksInstalled() bool {
	...
	// Verify they're beads hooks by checking for signature comment
	// #nosec G304 - controlled path from git directory
	preCommitContent, err := os.ReadFile(preCommit)
	if err != nil || !strings.Contains(string(preCommitContent), "beads (beads) pre-commit hook") {
		return false
	}

	// #nosec G304 - controlled path from git directory
	postMergeContent, err := os.ReadFile(postMerge)
	if err != nil || !strings.Contains(string(postMergeContent), "beads (beads) post-merge hook") {
		return false
	}
...
#
# beads (beads) pre-commit hook
#
# This hook ensures that any pending beads issue changes are flushed to
...
if ! command -v beads >/dev/null 2>&1; then
    echo "Warning: beads command not found, skipping pre-commit flush" >&2
    exit 0
fi
...
if ! beads sync --flush-only >/dev/null 2>&1; then
    echo "Error: Failed to flush beads changes to JSONL" >&2
    echo "Run 'beads sync --flush-only' manually to diagnose" >&2
    exit 1
fi
...
# Check if beads is available
if ! command -v beads >/dev/null 2>&1; then
    echo "Warning: beads command not found, skipping post-merge import" >&2
    exit 0
fi
...
if ! beads import -i .beads/issues.jsonl >/dev/null 2>&1; then
    echo "Warning: Failed to import beads changes after merge" >&2
    echo "Run 'beads import -i .beads/issues.jsonl' manually" >&2
...
`
	...
	for _, hookPath := range []string{preCommitPath, postMergePath} {
		if _, err := os.Stat(hookPath); err == nil {
			// Read existing hook to check if it's already a beads hook
			// #nosec G304 - controlled path from git directory
			content, err := os.ReadFile(hookPath)
			if err == nil && strings.Contains(string(content), "beads (beads)") {
				// Already a beads hook, skip backup
				continue
			}

			// Backup non-beads hook
...
func migrateOldDatabases(targetPath string, quiet bool) error {
	...
	if !quiet {
		green := color.New(color.FgGreen).SprintFunc()
		fmt.Fprintf(os.Stderr, "%s Database migration complete\n\n", green("✔"))
	}
