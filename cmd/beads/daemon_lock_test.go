...
func TestMultipleDaemonProcessesRace(t *testing.T) {
	...
	// Find the beads binary
	beadsBinary, err := exec.LookPath("beads")
	if err != nil {
		// Try local build
		if _, err := os.Stat("./beads"); err == nil {
			beadsBinary = "./beads"
		} else {
			t.Skip("beads binary not found, skipping race test")
		}
	}
	...
	// Initialize beads
	cmd = exec.Command(beadsBinary, "init", "--prefix", "test")
	cmd.Dir = tmpDir
	...
	for i := 0; i < numAttempts; i++ {
		go func() {
			cmd := exec.Command(beadsBinary, "daemon", "--interval", "10m")
			cmd.Dir = tmpDir
			cmd.Env = append(os.Environ(), "BEADS_DB="+dbPath)
			err := cmd.Start()
			if err != nil {
	...
	os.Remove(filepath.Join(beadsDir, "beads.sock"))
