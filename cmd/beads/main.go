...
var rootCmd = &cobra.Command{
	...
	PersistentPreRun: func(cmd *cobra.Command, args []string) {
		...
		if dbPath == "" {
			...
			if foundDB := beads.FindDatabasePath(); foundDB != "" {
				...
				if dbPath == "" {
					...
					fmt.Fprintf(os.Stderr, "Hint: run 'beads init' to create a database in the current directory\n")
		...
		// Priority: --actor flag > viper (config + BEADS_ACTOR env) > USER env > "unknown"
		// Note: Viper handles BEADS_ACTOR automatically via AutomaticEnv()
		if actor == "" {
			// Viper already populated from config file or BEADS_ACTOR env
		...
		// Skip if sync --dry-run to avoid modifying DB in dry-run mode (beads-191)
