	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(issues) == 0 && output != "" && !force {
			...
			if err != nil {
				...
				fmt.Fprintf(os.Stderr, "  beads export -o %s --force\n", output)
		...
		// Write JSONL (timestamp-only deduplication DISABLED due to beads-160)
		...
		// This prevents clearing dirty flags when exporting to custom paths (e.g., beads export -o backup.jsonl)
		if output == "" || output == findJSONLPath() {
			// Clear only the issues that were actually exported (fixes beads-52 race condition)
