...
func checkVersionMismatch() {
	...
	if dbVersion != Version {
		...
		fmt.Fprintf(os.Stderr, "%s\n", yellow(fmt.Sprintf("WARNING: Your bd binary (v%s) differs from the database version (v%s)", Version, dbVersion)))
		...
		if cmp < 0 {
			...
			fmt.Fprintf(os.Stderr, "%s\n\n", yellow("WARNING: Some features may not work correctly. Rebuild: go build -o bd ./cmd/bd"))
		} else if cmp > 0 {
...
func validateJSONLIntegrity(ctx context.Context, jsonlPath string) error {
	...
	if currentHash != storedHash {
		fmt.Fprintf(os.Stderr, "WARNING: WARNING: JSONL file hash mismatch detected (bd-160)\n")
...
func flushToJSONL() {
	...
	recordFailure := func(err error) {
		...
		if failCount >= 3 {
			...
			fmt.Fprintf(os.Stderr, "%s\n\n", red("WARNING: Run 'bd export -o .beads/issues.jsonl' manually to fix."))
		}
	}
