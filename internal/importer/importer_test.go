...
func TestImportIssues_Update(t *testing.T) {
	...
	if result.Updated == 0 && result.Created == 0 {
		t.Errorf("Expected some updates or creates, got Updated=%d Created=%d Skipped=%d Collisions=%d",
			result.Updated, result.Created, result.Skipped, result.Collisions)
	}
