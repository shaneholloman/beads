...
// TestRenderTier1Prompt_UTF8 verifies UTF-8 data integrity through AI API round-trip.
//
// IMPORTANT: This project ACTIVELY DISCOURAGES emoji use. Emojis add no business value,
// make code look unprofessional, and clutter documentation. However, we must handle
// emoji data without corruption if users choose to insert them despite this guidance.
//
// This test uses emojis as a stress test for multi-byte UTF-8 encoding through external
// API calls. It does NOT endorse emoji use - it ensures robust data handling regardless
// of input content.
//
// Preferred alternatives: Use plain text, markdown formatting, or text symbols (✔ ✘).
func TestRenderTier1Prompt_UTF8(t *testing.T) {
	...
	// Verify legitimate international character support
	...
	// Verify emoji data integrity (even though emoji use is DISCOURAGED)
	// This ensures we don't corrupt data if users ignore guidance
	if !strings.Contains(prompt, "🎉") {
		t.Error("prompt must not corrupt emoji data (even though emojis are discouraged)")
	}
	if !strings.Contains(prompt, "🚀") {
		t.Error("prompt must not corrupt emoji data (even though emojis are discouraged)")
	}
